...
// <hack>
	    ...
	    } else if (string_EQ_Q_ ("newFactFromRegister", func)) {
		var resolved_args=resolveArgs(args,bindings);
		var fact = AppendInefficient (resolved_args, registerAsList ());
		return newFact(fact);
	    } else if (string_EQ_Q_ ("addInRegister", func)) {
		var resolved_args=resolveArgs(args,bindings);
		register = car (resolved_args) + cadr (resolved_args);
		return register;
// </hack>
